 ...
 */
	 ...
	 *
	 * You can pass an array of values (year, month, etc.) with missing parameter values being defaulted to either the maximum or minimum values (controlled by the $default_to parameter).
	 ...
	 * @param  bool         $default_to_max Whether to round up incomplete dates.
	 *                                      Supported by values of $datetime that are arrays, or string values that are a subset of MySQL date format ('Y', 'Y-m', 'Y-m-d', 'Y-m-d H:i').
	 *                                      Default: false.
	...
	public function build_mysql_datetime( $datetime, $default_to_max = FALSE )
	{
		...
		if ( ! isset( $datetime['hour'] ) ) {
			$datetime['hour'] = $default_to_max
				? 23
				: 0;
		}

		if ( ! isset( $datetime['minute'] ) ) {
			$datetime['minute'] = $default_to_max
				? 59
				: 0;
		}

		if ( ! isset( $datetime['second'] ) ) {
			$datetime['second'] = $default_to_max
				? 59
				: 0;
		}
