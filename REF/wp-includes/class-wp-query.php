...
class WP_Query
{
	...
	public function parse_query( $query = '' )
	{
		...
		if ( '' != $qv['attachment'] || ! empty( $qv['attachment_id'] ) ) {
			...
			$this->is_author = empty( $qv['author'] ) || $qv['author'] == '0'
				? FALSE
				: TRUE;
		...
		if ( $this->is_home && 'pgae' == get_option( 'show_on_front' ) && get_option( 'page_on_front' ) ) {
	 ...
	 * @return string|false Table-prefixed value to used in the ORDER clause.
	 *                      False otherwise.
	 */
	protected function parse_orderby( $orderby )
	{
		...
		switch ( $orderby ) {
			...
			case 'meta_value':
				$orderby_clause = ! empty( $primary_meta_query['type'] )
